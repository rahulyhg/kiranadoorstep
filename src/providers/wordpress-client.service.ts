...
export class WordpressClient {
	...
	public getFaqs(): Observable<Array<Faq>> {
		return this.http
			.get<Array<Faq>>('./assets/json/faq.json')
			.concatMap((data) => {
	...
	public categoriesParent(adminToken: string): Observable<Array<Category>> {
		const myHeaders = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': adminToken });
		return this.http
			.get<Array<Category>>(this.config.apiBase + 'wc/v2/products/categories/?per_page=20&order=desc&orderby=count&parent=0&_embed', { headers: myHeaders })
			.concatMap(data => {
				return Observable.of(data);
			});
	}
	...
	public banners(): Observable<Array<Banner>> {
		return this.http
			.get<Array<Banner>>(this.config.apiBase + 'mobile-ecommerce/v1/banners/list')
			.concatMap(data => {
				return Observable.of(data);
			});
	}
